...
class BillAdapter (private val list: List<Bill>) : RecyclerView.Adapter<BillAdapter.viewHolder>() {
    var onClickItem : (Bill, Int) -> Unit = { _, _ -> }
    var onClickOrderAgain : (Bill, Int) -> Unit = { _, _ -> }
    inner class viewHolder (private val binding: BillItemBinding) : RecyclerView.ViewHolder(binding.root) {
        init {
            ...
            binding.btnOrderAgain.setOnClickListener {
                onClickOrderAgain.invoke(list[adapterPosition], adapterPosition)
            }
        }
        fun onbind(bill: Bill) {
            ...
            when (bill.status) {
                "Pending" -> {
                    ...
                    binding.btnOrderAgain.visibility = ViewGroup.GONE
                    binding.textView24.visibility = ViewGroup.GONE
                }
                "Success" -> {
